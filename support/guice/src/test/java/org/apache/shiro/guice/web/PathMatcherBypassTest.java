...
class PathMatcherBypassTest {
    ...
    void guicePathBypass() {
        ...
        Key<? extends Filter>[] adminChain = (Key<? extends Filter>[]) new Key<?>[]{ Key.get(Filter.class) };
        Map<String, Key<? extends Filter>[]> chains = new LinkedHashMap<>();
        chains.put("/admin/foo", adminChain);

        Injector injector = mock(Injector.class);
        var resolver = new SimpleFilterChainResolver(chains, injector, matcher);
        ...
        FilterChain r2 = resolver.getChain(req2, mock(HttpServletResponse.class), orig);

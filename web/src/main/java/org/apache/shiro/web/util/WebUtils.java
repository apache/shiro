...
public class WebUtils {
	 ...
	 * request and later restore it, for example when redirecting to a requested
	 * page after login, equal to {@code shiroSavedRequest}.
	 */
	...
	/**
	 * Return the path within the web application for the given request. Detects
	 * include request URL if called within a RequestDispatcher include.
	 * <p/>
	 * For example, for a request to URL
	 * <p/>
	 * <code>http://www.somehost.com/myapp/my/url.jsp</code>,
	 * <p/>
	 * for an application deployed to <code>/mayapp</code> (the application's
	 * context path), this method would return
	 * <p/>
	 ...
	 * <p/>
	 * This implementation rethrows an exception that happened on environment
	 ...
	 * @return the desired WebEnvironment for this web app, or <code>null</code>
	 *         if none
	 * @since 1.2
	 */
	...
	/*
	 * Returns {@code true} IFF the specified {@code SubjectContext}: <ol> <li>A
	 * {@link WebSubjectContext} instance</li> <li>The {@code
	 * WebSubjectContext}'s request/response pair are not null</li> <li>The
	 * request is an {@link HttpServletRequest} instance</li> <li>The response
	 * is an {@link HttpServletResponse} instance</li> </ol>
	 *
	 * @param context the SubjectContext to check to see if it is HTTP
	 * compatible.
	 * 
	 * @return {@code true} IFF the specified context has HTTP request/response
	...
	/**
	 * Returns {@code true} if a session is allowed to be created for a
	 * subject-associated request, {@code false} otherwise.
	 * <p/>
	 * <b>This method exists for Shiro's internal framework needs and should
	 * never be called by Shiro end-users. It could be changed/removed at any
	 * time.</b>
	 *
	 * @param requestPairSource
	 *            a {@link RequestPairSource} instance, almost always a
	 *            {@link org.apache.shiro.web.subject.WebSubject WebSubject}
	 *            instance.
	 * @return {@code true} if a session is allowed to be created for a
	 *         subject-associated request, {@code false} otherwise.
	 */
	 ...
	 * @param queryParams
	 *            a map of parameters that should be set as request parameters
	 *            for the new request.
	 * @param contextRelative
	 *            true if the URL is relative to the servlet context path, or
	...
	public static void issueRedirect(ServletRequest request, ServletResponse response, String url, Map queryParams,
			boolean contextRelative, boolean http10Compatible) throws IOException {
	...
	/**
	 * <p>
	 * Checks to see if a request param is considered true using a loose
	 * matching strategy for general values that indicate that something is true
	 * or enabled, etc.
	 * </p>
	 * <p/>
	 * <p>
	 * Values that are considered "true" include (case-insensitive): true, t, 1,
	 * enabled, y, yes, on.
	 * </p>
	 *
	 * @param request
	 ...
	 * @return true if the given parameter is considered "true" - false
	 *         otherwise.
	 */
	 ...
	 * {@link #saveRequest(javax.servlet.ServletRequest) saved} request, or if
	 * there is no saved request, redirects the end user to the specified
	 * {@code fallbackUrl}. If there is no saved request or fallback url, this
	 * method throws an {@link IllegalStateException}.
	 * <p/>
	 * This method is primarily used to support a common login scenario - if an
	 * unauthenticated user accesses a page that requires authentication, it is
	 * expected that request is
	 * {@link #saveRequest(javax.servlet.ServletRequest) saved} first and then
	 * redirected to the login page. Then, after a successful login, this method
	 * can be called to redirect them back to their originally requested URL, a
	 ...
	 * @param fallbackUrl
	 *            the fallback url to redirect to if there is no saved request
	 *            available.
	 * @throws IllegalStateException
	 *             if there is no saved request and the {@code fallbackUrl} is
	 *             {@code null}.
	 * @throws IOException
	 *             if there is an error redirecting
	 * @since 1.0
	 */
